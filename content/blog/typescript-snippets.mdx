...
T extends object ? { [P in keyof T]?: DeepPartial\<T[P]\1>; } : T; 这是一个条件类型（conditional type）的定义。它检查 T 是否是一个对象类型，如果是，则将对象的每个属性变为可选属性，并递归地应用 DeepPartial 类型到每个属性的值上；如果不是对象类型，则返回 T 自身。
