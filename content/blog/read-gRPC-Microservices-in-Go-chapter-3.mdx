...
两个服务使用协议缓冲区（protocol buffers）来交换消息

协议缓冲区允许你序列化结构化数据，以便通过网络传输。你还可以定义服务功能并生成特定语言的源代码。消息和服务功能的定义写在一个称为.proto文件的配置文件中，该文件还包含我们将在本书中使用的协议版本信息（proto3）


定义一个CreateOrderRequest消息格式，每个CreateOrderRequest包含user_id、items和amount。在这种情况下，.proto文件的内容会像这样：

```protobuf
syntax = "proto3";                // 协议版本
message CreateOrderRequest {
  int64 user_id = 1;             // 此订单的所有者
  repeated Item items = 2;       // 此订单的物品列表
  float amount = 3;              // 应支付的此订单的总金额
}
```

消息字段可以是以下之一：
- 单一（Singular）—一个结构化消息最多可以有一个这样的字段。这个字段是默认的字段规则。
- 重复（Repeated）—任何具有此规则的字段都可以包含多个值，包括零个。这些项目的顺序被保留。在前面的示例中，消息中可以有多个item值（例如，一个客户同时订购了多个产品）。

由于Protobuf编译器生成多种语言的源代码，它依赖于这些规则来要求字段命名约定。字段名称应该是小写的；如果包含多个单词，应该用下划线分隔（例如，user_id）。

每个字段在消息的二进制消息格式中都有一个唯一的标识符，以识别字段。由于字段编号是字段的唯一标识符，这些编号不应更改，以提供向后兼容性保证。如果你想删除一个字段，最好在删除它之前使用reserved关键字来保留它，以防止将来用相同的字段编号或名称定义它。你也可以使用to关键字通过单个或范围的字段编号来保留这些字段。例如，删除字段编号为3的customer_id字段，并添加具有相同字段名称或编号但类型不同的新字段，
```protobuf
message CreateOrderRequest {
